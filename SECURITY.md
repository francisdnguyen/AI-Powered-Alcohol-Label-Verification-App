...
| **Denial of wallet** — runaway Claude spend | Governed by an **account-level usage limit in the Anthropic Console** (a per-app in-memory cap can't be enforced across serverless instances, so the reliable ceiling lives with the account). The per-IP rate limit below bounds request volume; batch concurrency is also bounded. |
...
- **In-memory state is per-instance.** The rate-limit counters and mock applications live in a
  `Map`. On Vercel each serverless instance has its own copy and it resets on cold start — so the
  rate limit is **per-instance, not globally enforced**. Production would move these to a shared
  store (Vercel KV / Redis / Postgres). (Batch no longer
